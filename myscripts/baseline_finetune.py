...
class JvsSpeechDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, id):
        ...
        nopad_mel = whisper.log_mel_spectrogram(audio.flatten())
        audio = whisper.pad_or_trim(audio.flatten())
        mel = whisper.log_mel_spectrogram(audio)
        f_len = nopad_mel.shape[-1]
        # import pdb
        # pdb.set_trace()
...
class Config:
    learning_rate = 5e-6 #5e-7 #5e-6 #0.0005
    weight_decay = 0.01
    adam_epsilon = 1e-8
    warmup_steps = 2
    batch_size = 8  #4 #8 #8 #16
    num_worker = 4 #2
    num_train_epochs = 10 #10
    gradient_accumulation_steps = 2 #4 #2 #4 #2 #1
...
class WhisperModelModule(LightningModule):
    ...
    def training_step(self, batch, batch_id):
        ...
        #batch_size = batch['label'].length.size(-1)
        # with torch.no_grad():
        #     audio_features, ht = self.teacher.encoder(input_ids)
        #     #ht = self.teacher.encoder.blocks[11]
        ...
        label = batch['label']
        s_ce_loss = self.ce_loss2(e2e_out.tensor.reshape(-1, e2e_out.tensor.size(-1)), label.tensor.reshape(-1))
        # import pdb
        # pdb.set_trace()
        ...
        # e2e_out and label's id=-1 不能算loss
        ...
        #     loss = s_ce_loss + ts_loss * self.lam1
        # else:
    ...
    def validation_step(self, batch, batch_id):
        ...
        for o, l in zip(e2e_out, label):
            ...
            # print("hyp: ", decode_text(o))
            # print("ref: ", decode_text(l))
...
def train():
    ...
    train_audio_transcript_pair_list = get_audio_file_list_from_csv("data/train.csv")
...
def inference(checkpoint_path): 
    ...
    hyp_path = os.path.join("whisper-" + scale, cfg.mode, "decode")
...
def main():
    checkpoint = "whisper-" + scale + "/finetune/artifacts/checkpoint/last-v1.ckpt"
    #train()
