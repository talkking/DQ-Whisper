...
class QuantConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1,
                 bias=False, bit=4, power=True, additive=False, q_activation=False, grad_scale=None):
...
class last_fc_new(nn.Linear):
    def __init__(self, in_features, out_features, bias,bit, power=True, additive=False, q_activation=False, grad_scale=None):
        super(last_fc_new, self).__init__(in_features, out_features, bias)#, bit, power, additive, q_activation, grad_scale)
        self.layer_type = 'LFC'
        self.bit = bit
        self.weight_alpha = torch.nn.Parameter(torch.tensor(3.0))
        self.q_activation = q_activation 
    ...
    def forward(self, x):
       bit = (1 << (self.bit - 1)) - 1
       max = self.weight.data.max()
       weight_q = self.weight.div(max).mul(bit).round().div(bit).mul(max)  #模拟权重8bit均匀量化
       weight_q = (weight_q - self.weight).detach() + self.weight #weight_q的梯度不反传
       return F.linear(x, weight_q.half(), self.bias.half() if self.bias is not None else self.bias) 
       #return F.linear(x, weight_q, self.bias)
        ...
        #     if self.q_activation:
        #         x = uniform_quantization(x, self.act_alpha, self.bit, False, self.grad_scale)
        # #return F.linear(x, weight.half(), self.bias.half() if self.bias is not None else self.bias) 
